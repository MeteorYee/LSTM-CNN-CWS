...
def seg_sequence(graph, tX, batch_size):
    ...
    label_node = graph.get_operation_by_name("prefix/Reshape_5")
    label_scores = label_node.outputs[0]
    label_unary_score = tf.argmax(label_scores, 2)
    ...
    with tf.Session(graph = graph) as sess:
        ...
        for i in xrange(numBatch):
            ...
            unary_scores = sess.run(label_unary_score, feed_dict)

            for unary_score in unary_scores:
                results.append(unary_score)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      "--model_path",
      type = str,
      default = "Models/lstm_cnn_model.pbtxt",
      help = "model path")
    parser.add_argument(
      "--test_data",
      type = str,
    ...
    parser.add_argument(
      "--result_path",
      type = str,
      default = "Results/cnn_result.txt",
